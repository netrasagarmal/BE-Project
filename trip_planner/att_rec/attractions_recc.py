#import os
#import sys 
#!{sys.executable} -m pip install rbm
#pip install rbm
import pandas as pd
import numpy as np
...
#C:\Users\mspat\Downloads\rbm-0.0.3-py3-none-any.whl
from .rbm import RBM
...
def get_recc(att_df, cat_rating):
    ...
    rbm.export(unseen, seen, 'G:/ITRS/rbm_models/'+filename, str(user))
    return filename, user, rbm_att

def filter_df(filename, user, low, high, province, att_df):
    recc_df = pd.read_csv('G:/ITRS/rbm_models/'+filename+'/user{u}_unseen.csv'.format(u=user), index_col=0)
    ...
    url = pd.read_json('G:/ITRS/outputs/attractions_cat.json',orient='records')
    url['id'] = url.index
...
def get_image(name):
    ...
    try:
        ...
        for filename in glob.glob("G:/ITRS/downloads/{name}/*jpg".format(name=name)) + glob.glob("downloads/{name}/*png".format(name=name)):
            return filename
    except:
        for filename in glob.glob("G:/ITRS/downloads/*jpg"):
            return filename
