...
def get_recc(att_df, cat_rating):
    ...
    rbm.export(unseen, seen, 'D:/ITRS/rbm_models/'+filename, str(user))
    return filename, user, rbm_att

def filter_df(filename, user, low, high, province, att_df):
    recc_df = pd.read_csv('D:/ITRS/rbm_models/'+filename+'/user{u}_unseen.csv'.format(u=user), index_col=0)
    ...
    url = pd.read_json('D:/ITRS/outputs/attractions_cat.json',orient='records')
    url['id'] = url.index
...
def get_image(name):
    ...
    try:
        ...
        for filename in glob.glob("D:/ITRS/downloads/{name}/*jpg".format(name=name)) + glob.glob("downloads/{name}/*png".format(name=name)):
            return filename
    except:
        for filename in glob.glob("D:/ITRS/downloads/*jpg"):
            return filename
